...
with open(LOG_PATH, "w", encoding="utf-8") as f:
        f.write("timestamp,pid,proc_name,score,label\n")
...
def process_and_finalize(key, flow):
    ...
    # label = "Suspicious" if probs[0][0] > SUSPICIOUS_THRESHOLD else "Normal"
    label = CLASS_LIST[np.argmax(probs)]
    score = probs.max()
    safe_score = probs[0][0]
    print(f"[{time.strftime('%H:%M:%S')}] Поток {key}: процесс={proc_name}, score={score:.3f}, метка={label}")

    ts = time.strftime("%Y-%m-%d %H:%M:%S")
    line = f"{ts},{pid or ''},{proc_name},{safe_score:.6f},{label}\n"
