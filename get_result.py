...
class HybridSecuritySystem:
    def __init__(self, models_dir="./"):
        ...
        self.xgb_clf = joblib.load(os.path.join(models_dir, "xgb_model.joblib"))
        self.xgb_scaler = joblib.load(os.path.join(models_dir, "scaler.joblib"))
        
        # 2. Загрузка Autoencoder (AE)
        self.ae_scaler = joblib.load(os.path.join(models_dir, "scaler_ae.pkl"))
        self.ae_threshold = np.load(os.path.join(models_dir, "threshold_ae.npy"))
        self.ae_model = NetworkAE(input_dim=17)
        self.ae_model.load_state_dict(torch.load(os.path.join(models_dir, "network_ae.pth")))
        self.ae_model.eval()

        # 3. Загрузка Isolation Forest (IF) - имена из вашего кода
        self.if_model = joblib.load(os.path.join(models_dir, "isolation_forest.pkl"))
        self.if_scaler = joblib.load(os.path.join(models_dir, "scaler_if.pkl"))
        self.if_threshold = np.load(os.path.join(models_dir, "threshold_if.npy"))

        # 4. Загрузка Local Outlier Factor (LOF) - имена из вашего кода
        self.lof_model = joblib.load(os.path.join(models_dir, "lof_model.pkl"))
        self.lof_scaler = joblib.load(os.path.join(models_dir, "scaler_lof.pkl"))
        self.lof_threshold = np.load(os.path.join(models_dir, "threshold_lof.npy"))

        # 5. Мета-модель
        self.meta_model = joblib.load(os.path.join(models_dir, "meta_model.joblib"))
        
        # Инициализация калибраторов
        self.cal_ae = ScoreCalibrator(self.ae_threshold)
        self.cal_if = ScoreCalibrator(self.if_threshold)
        self.cal_lof = ScoreCalibrator(self.lof_threshold)

    def assign_final_threat_status(self, threat_prob, anomaly_consensus):
        """Алгоритм принятия решения по финальной метрике опасности"""
        if threat_prob > 0.9 and anomaly_consensus > 0.7:
            return "CRITICAL: Known Attack Verified by Anomaly"
        elif threat_prob > 0.8:
            return "HIGH: Potential Known Attack"
        elif anomaly_consensus > 0.85:
            return "HIGH: Unknown Zero-Day Anomaly"
        elif threat_prob > 0.5 or anomaly_consensus > 0.5:
            return "WARNING: Suspicious Activity"
        else:
            return "NORMAL: Verified Legitimate"
    ...
    def instance_analyze_flow(self, flow_features):
        ...
        prob_malicious = 1.0 - xgb_probs[0] # Вероятность НЕ Normal Traffic
        prob_malicious = prob_malicious[0]
