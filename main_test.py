...
def decide_src_dst(local_ip, local_port, remote_ip, remote_port, direction):
    ...
    # если ни один не наш — не наш трафик
    if not host_l and not host_r:
        return None

    # главный принцип: чей IP наш — тот local
...
class NetworkPacket:
    """
    Fields from ETW C#: ID|PID|Process|Proto|Dir|LocalIP|LocalPort|RemoteIP|RemotePort|Bytes|TsMicros
    After decide_src_dst we keep normalized src/dst and ports as ints.
    """
    ...
    with flows_lock:
        ...
        if flow is None:
            flow = {
                ...
                # first observed destination port (feature)
